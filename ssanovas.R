## Fit ssanova model
ssanovas <- function(formula,type=NULL,data=list(),weights,subset,
                    ...
                    skip.iter=FALSE)
{
    ...
    else z <- mspreg1s(s,r,id.basis,y,wt,method,alpha,varht,random,skip.iter)
...
mspreg1s <- function(s,r,id.basis,y,wt,method,alpha,varht,random,skip.iter)
{
    qr.trace <- FALSE
    if ((alpha<0)&(method%in%c("u","v"))) qr.trace <- TRUE
    alpha <- abs(alpha)
    ...
    repeat {
        zz <- nlm(cv.wk,theta,stepmax=1,ndigit=7,iterlim=25)
        if (zz$code<=4)  break
        theta <- zz$est
        counter <- counter + 1
        if (counter>=5) {
